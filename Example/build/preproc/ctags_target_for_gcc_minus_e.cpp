# 1 "C:\\Users\\SVR-DEV04\\Documents\\Proyectos\\Dispensadora_E\\Example\\Example.ino"
# 2 "C:\\Users\\SVR-DEV04\\Documents\\Proyectos\\Dispensadora_E\\Example\\Example.ino" 2
# 3 "C:\\Users\\SVR-DEV04\\Documents\\Proyectos\\Dispensadora_E\\Example\\Example.ino" 2
# 4 "C:\\Users\\SVR-DEV04\\Documents\\Proyectos\\Dispensadora_E\\Example\\Example.ino" 2
...
void buzzHandle(int millis, byte pulses) {
  for (byte i = 0; i < pulses; i++) {
    digitalWrite(buzz_pin, 0x1);
    delay(millis / 2);
    digitalWrite(buzz_pin, 0x0);
    delay(millis / 2);
  }
}
...
void setup()
{
    ...
    for (byte i = 0; i < sizeof(cstmr_btns); i ++) {
        pinMode(cstmr_btns[i], 0x2);
    }

    lcd.init();
    lcd.backlight();
    lcd.clear();
    pinMode(buzz_pin, 0x1);
    pinMode(coin_btn, 0x2);
    pinMode(prog_btn, 0x2);
}
